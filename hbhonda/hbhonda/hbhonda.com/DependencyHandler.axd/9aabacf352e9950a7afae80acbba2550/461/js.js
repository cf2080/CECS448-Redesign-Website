...
// Main Image Height
var slideshowContainer = $('#slideshow-main');

function vehicleDiscountHeight() {
    if (!$(".discount-details").get(0)) {
        return 0;
    }
    var height = $(".discount-details").map(function (idx, elm) {
        return $(elm).outerHeight(true);
    }).toArray().reduce(function (curr, prev) {
        return prev > curr ? prev : curr;
    }, 0);
    return height >= 0 ? height : 0;
}

function SlideshowMainHeight() {
    var frameHeight = $('.model-photos .thumbnail-frame').outerHeight();
    var thumbnailsHeight = $('#slideshow-thumbnails').outerHeight();
    var discountHeight = vehicleDiscountHeight();
    var slideshowHeightCSS = (Math.ceil(frameHeight - thumbnailsHeight - discountHeight)) + "px";
    slideshowContainer.css("height", slideshowHeightCSS);
}

// Owl Slider Max Height
var sliderMainMaxHeight = function (slider, percentage) {
    var $slider = $(slider);
    var width = $(".owl-item", $slider).width();
    var percentage;
    var height;
    ...
    if ($slider.find("iframe").length > 0) {
        height = $slider.find("iframe").height();
    } else {
        height = width * percentage;
    }

    $(".owl-item img", slider).each(function () {
        if ($(this).height() > 0) { // image has loaded
            if ($(this).height() > $(this).width()) {
                $(this).addClass("tall-img").css("height", height);
            }
        } else {
    ...
    $(".owl-item .item-video", slider).each(function (index, value) {
        var video = $(value);
...
// Position image counter on sliders
var posSliderImgCounter = function (thumbnailSlider) {
    var $thumbnailSlider = $(thumbnailSlider);
    var parentHeight = $thumbnailSlider.parent().outerHeight();    
    var slideshowMainHeight = $thumbnailSlider.parent().find("#slideshow-main").outerHeight();    
    $thumbnailSlider.parent().find(".image-count").css("bottom", parentHeight - slideshowMainHeight);
}

// Position slider controls
var posSliderControls = function (slider) {
    var $slider = $(slider);
    var sliderHeight = $slider.outerHeight();
    var controlsHeight = $slider.find(".owl-prev").outerHeight();

    $slider.find(".owl-nav > div").css("top", (sliderHeight - controlsHeight) / 2);
}

// Center images in sliders
var posAlignImgVertically = function (slider) {
    var $slider = $(slider);
    var containerHeight = $slider.outerHeight();

    $(".owl-item", $slider).each(function () {
        $(this).css("top", (containerHeight - $(this).outerHeight()) / 2);
    });
}

var initOwlCarouselSlider = function () {

    // Owl Slider
    var sliderStatus = $("#sliderStatus");

    // Update Total picture. For example 1/12
    function updateResult(pos, value) {
        sliderStatus.find(pos).find(".result").text(value);
    }

    var sync1 = $("#slideshow-main");
    sync1.on("initialized.owl.carousel", function (event) {
        updateResult(".owlItems", event.item.count);
        updateResult(".currentItem", 1);
    });
    ...
    sync1.owlCarousel({
        ...
        slideBy: 1,
        smartSpeed: 100,
        nav: sync1.children().length > 1,
        loop: sync1.children().length > 1,
        mouseDrag: false,
        pullDrag: false,
        callbacks: true,
        lazyLoad: true,
        video: true,
        navText: [
            "<span class='thumb-prev'><span class='fa fa-chevron-left'></span></span>",
            "<span class='thumb-next'><span class='fa fa-chevron-right'></span></span>"
        ]
    });

    sync1.on("changed.owl.carousel", function (event) {
        updateResult(".currentItem", event.page.index + 1);
        ...
        syncThumbnails(event);
    });

    var syncThumbnails = function (event) {
        sync2.find(".synced").removeClass("synced");
        $(sync2.data("owl.carousel").$element[0]).find("div:first div:first").children().eq(event.page.index).addClass("synced");
        var thumbnailPage = (Math.ceil((event.page.index + 1) / 5) - 1);
        sync2.trigger("to.owl.carousel", [thumbnailPage, 100]);
    }

    var sync2 = $("#slideshow-thumbnails");
    sync2.on("initialized.owl.carousel", function (event) {
        $(event.target).find(".active").eq(0).addClass("synced");
    });
    ...
    $("#slideshow-thumbnails").on("click", ".owl-item", function (e) {
        ...
        sync1.trigger("to.owl.carousel", [number, 200]);
    });
    ...
    $(table).each(function () {
        ...
        if (isBreakpoint('sm') || isBreakpoint('md') || isBreakpoint('lg')) { // wrap cols in div.striperows
            ...
            if (assembly.length > 0) {
                col = 1;
                assembly.removeClass("col-sm-6");
                assembly.addClass("col-sm-12");
            }
            ...
            if (assembly.length > 0) {
                assembly.removeClass("col-sm-12");
                assembly.addClass("col-sm-6");
            }
        }
        //if (isBreakpoint('lg')) { // wrap every 3 cols in div.striperows
        //    for (var i = 0; i < $divs.length; i += 3) {
        //        var $wrapper = $('<div class="striperows"/>');
        //        $divs.slice(i, i + 3).wrapAll($wrapper);
        //    }
        //}
    });
}
...
var splitLists = function (list, columns) {
...
var splitAvailableModels = function () {
    ...
    if ($container.outerWidth() >= 768 && $container.outerWidth() < 992) {
        splitLists('.availablemodels.split-list', 2);
    }

    if ($container.outerWidth() >= 992) {
        splitLists('.availablemodels.split-list', 3);
    }
}
...
$(document).ready(function () {
    ...
    $('#specs-container').easyResponsiveTabs();

    $(".view-all-available-color-link").on("click",
        function (e) {
            var $container = $(e.currentTarget).closest("div[itemtype=\"http://schema.org/Product\"]");
            $container.find(".availablemodels-tab").trigger("click");
        });
    ...
    (function ($) {
        ...
        // have to do both activeAccordion and activeTab because of breakpoints
        var activeAccordion = $('.resp-accordion:visible:first').attr('aria-controls');
        $('li[aria-controls="' + activeAccordion + '"]').addClass('resp-tab-active');
        $('h2[aria-controls="' + activeAccordion + '"]').addClass('resp-tab-active');
        $('div[id="' + activeAccordion + '"]').addClass('resp-tab-content-active');

        var activeTab = $('.resp-tab-item:visible:first').attr('aria-controls');
        $('li[aria-controls="' + activeTab + '"]').addClass('resp-tab-active');
        $('h2[aria-controls="' + activeTab + '"]').addClass('resp-tab-active');
        $('div[id="' + activeTab + '"]').addClass('resp-tab-content-active');
    }(jQuery));
});
