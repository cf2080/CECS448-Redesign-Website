...
(function () {
    ...
    function setPrintLeadInfoData($elm, val, data, questionMappedKey) {
    ...
    function buildFormJsonRequestMessage(clientId) {
        var $root = $("#" + clientId);
        if (!$root.get(0)) {
            return null;
        }
        var data = {
            FormName: $root.find("[data-form-name]").attr("data-form-name")
        };
        $root.find("[data-question-mapped-key-name]").each(function (idx, elm) {
            ...
            if (!data[questionMappedKey] && data[questionMappedKey] !== "" && ["Comment", "JointApplicantComment"].indexOf(questionMappedKey) === -1) {
                ...
                setPrintLeadInfoData($elm, val, data, questionMappedKey);
            }
        });
        return data;
    }
...
function Dx1ViewForm(clientId, options) {
    ...
    function activateAndFillDropdowWithData(ddl, data, defaultValue) {
        ...
        if ($ddl.get(0) === self.modelDropDown) {
            if (self.options.productId !== "00000000-0000-0000-0000-000000000000") {
        ...
        return callAjaxToGetModelLookUpData(url)
            .then(function (response) {
                if (response.IsSuccess) {
                    activateAndFillDropdowWithData(
                        self.manufacturerDropDown,
    ...
    function initModelControlValue() {
        ...
        return $.when(populateManufacturerDropdown(selectedModelJson.category)
            , populateYearDropdown(
                selectedModelJson.category
                , selectedModelJson.manufacturer)
            , populateModelDropdown(
                selectedModelJson.category
                , selectedModelJson.manufacturer
                , selectedModelJson.year))
            .then(function () {
                setDropDownValue(
                    self.manufacturerDropDown
                    , selectedModelJson.manufacturer);
    ...
    function handleErrorSubmitForm(button, submitText, isInvalidRecaptcha) {
        ...
        button.val(submitText);
    }

    function getFormData(opts) {
        var data = ViewFormUtil.buildFormJsonRequestMessage(self.clientId);
        if (opts.productId && opts.productId !== "00000000-0000-0000-0000-000000000000") {
            data.ProductId = opts.productId;
            data.CurrentPrice = opts.currentPrice;
        }

        if (opts.dealershipId && opts.dealershipId !== "00000000-0000-0000-0000-000000000000") {
        ...
        data.IsSendToShiftDigital = opts.isSendToShiftDigital;
        return data;
    }
    ...
    function sendFormSubmissionEventToShiftDigitalAnalytics(formName, relativeUrl, opts) {
        if (!window.sd || !formName) {
            return;
        }
        if (relativeUrl && relativeUrl.toLowerCase().indexOf("postfinanceapplicationlead") > -1 &&
            opts && opts.productId === "00000000-0000-0000-0000-000000000000") {
            return;
        }
        if (opts && opts.productId !== "00000000-0000-0000-0000-000000000000" && !opts.isPolarisBrand) {
            return;
        }
        var data = {
            formType: formName,
    ...
    function submitForm() {
        ...
        var submitText = button.val();
        button.val(opts.submittingText);
        container.addClass(opts.submittingClass);
        ...
        $.ajax({
            ...
            error: function (jqXhr, textStatus, errorThrown) {
                ...
                handleErrorSubmitForm(button, submitText, null);
                deferred.reject();
            },
            success: function (response) {
                if (!response.Success) {
                    handleErrorSubmitForm(button, submitText, response.IsInvalidRecaptcha);
                    return;
                }
                sendFormSubmissionEventToShiftDigitalAnalytics(formName, relativeUrl, opts);
                if (opts.finalUrl) {
                    window.location = opts.finalUrl;
                } else if (opts.finalMessage) {
                    var finalMessage = opts.finalMessage;
                    if (response.Message) {
                        finalMessage = response.Message;
                    }
                    if ($('.formControl').length) {
                        var formControl = container.find('.formControl');
                        var modalContent = formControl.closest(".modal-content");
                        var modalFooter = modalContent.find(".modal-footer");
                        modalFooter.find('.submit-button').hide();
                        container.find('.formControl').hide();
                    } else {
                        container.find('.section-wrap').hide();
                        container.find('input[type="button"]').hide();
                        container.find('.form-title').hide();
                    }
                    if (!container.find('.em-message').length) {
                        container.append('<p class="em-message"></p>');
                    }
                    container.find('.em-message').html(finalMessage).show();
                    sendGoogleAnalyticForFormSubmission();
                    container.find('.secure-image').hide();
                }
                deferred.resolve();
            }
        });
    ...
    function getValidationControl(selector) {
        ...
        // finding Muti Choice
        if (!$control.length && $container.find(".multi-choice").length > 0) {
            $control = $container.find(".multi-choice").find(":checkbox, :radio");
        }
    ...
    function initCustomValidatorMethods() {
        jQuery.validator.addMethod("dxRequired",
        ...
        jQuery.validator.addMethod("dxEmail",
            function (value) {
                if (!value || !value.trim()) {
                    return true;
                }
        ...
        jQuery.validator.addMethod("dxPhone",
            function (value) {
                if (!value || !value.trim()) {
                    return true;
                }
            ...
            "Phone number should only consist of numbers, ( ), -, +");

        jQuery.validator.addMethod("dxDate",
            function (value) {
                if (!value || !value.trim()) {
                    return true;
                }
    ...
    function initControls() {
        ...
        if (container.find(".datepicker").length > 0) {
            var currentYearPlusTen = (new Date()).getFullYear() + 10;
        ...
        container.find(".rating-star").on("click", function () {
            $(this).parents(".dx1-rating").find(".rating-star").removeClass("checked");
            $(this).addClass('checked');

            var submitStars = $(this).attr('data-value');
            $(this).parents(".dx1-rating").find(":button").val(submitStars);
            $(this).parents(".dx1-rating").find(":button").valid();
        });
